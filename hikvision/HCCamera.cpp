...
bool LicenseSnapResult(const NET_DVR_ALARMER *pAlarmer, const char *pAlarmInfo) {
    ...
    char dst_utf8[16] = {0};
    GbkToUtf8(struITSPlateResult.struPlateInfo.sLicense, strlen(struITSPlateResult.struPlateInfo.sLicense), dst_utf8, sizeof(dst_utf8));

    printf("---Plate License: %s\n", dst_utf8);//³µÅÆºÅ
    printf("strlen(struITSPlateResult.struPlateInfo.sLicense) %d\n",strlen(struITSPlateResult.struPlateInfo.sLicense));
    ...
    if (nBkTag != -1 && nLicenseTag != -1) {
        ...
        if (plistBuffer) {
            ...
            memcpy(&plistBuffer->camerasPic.tCameraTime,(char *) struITSPlateResult.struPicInfo[nBkTag].byAbsTime, 32);

            //车辆颜色 保留字段 暂无
            plistBuffer->camerasPic.nLicense_car_color=struITSPlateResult.struVehicleInfo.byColor;

            //printf("plistBuffer->camerasPic.nLicense_car_color=%d\n",plistBuffer->camerasPic.nLicense_car_color);//车辆颜色必须要转换才正确
            //车牌类型

            plistBuffer->camerasPic.nLicense_plate_type=struITSPlateResult.byVehicleType;
            int cart;
            cart= plistBuffer->camerasPic.nLicense_plate_type;

            if(cart==VTR_RESULT_OTHER)
            {
                plistBuffer->camerasPic.nLicense_plate_type=VCA_OTHER_TYPE;
            }
            else if(cart==VTR_RESULT_BUS||cart==VTR_RESULT_MEDIUM_BUS)
            {
                plistBuffer->camerasPic.nLicense_plate_type=VCA_BIGCAR_TYPE;
            }
            else if(cart==VTR_RESULT_TRUCK)
            {
                plistBuffer->camerasPic.nLicense_plate_type=VCA_BIGCAR_TYPE;
            }
            else if(cart == VTR_RESULT_CAR||cart==VTR_RESULT_SUV_MPV||cart==VTR_RESULT_SMALLCAR||cart==VTR_RESULT_MICROCAR||cart==VTR_RESULT_HATCHBACK||cart==VTR_RESULT_SALOON||
                    cart==VTR_RESULT_SPORT_SEDAN)
            {
               // plistBuffer->camerasPic.nLicense_plate_type=VCA_CAR_TYPE;
                plistBuffer->camerasPic.nLicense_plate_type=VCA_SMALLCAR_TYPE;//轿车也是小型车
            }
            else if(cart==VTR_RESULT_MINIBUS)
            {
                plistBuffer->camerasPic.nLicense_plate_type=VCA_SMALLCAR_TYPE;
            }
            else if(cart==VTR_RESULT_SMALLTRUCK)
            {
                plistBuffer->camerasPic.nLicense_plate_type=VCA_SMALLCAR_TYPE;
            }
            else if(cart==VTR_RESULT_PICKUP||cart==VTR_RESULT_MINI_TRUCK||cart==VTR_RESULT_TUMBREL||cart==VTR_RESULT_TRIKE||cart==VTR_RESULT_NON_MOTOR_VEHICLE)
            {
                plistBuffer->camerasPic.nLicense_plate_type=VCA_SMALLCAR_TYPE;//小型车
            }
            else if(cart==VTR_RESULT_CONTAINER_TRUCK||cart==VTR_RESULT_SLAG_CAR||cart==VTR_RESULT_CRANE||cart==VTR_RESULT_OIL_TANK_TRUCK||cart==VTR_RESULT_CONCRETE_MIXER
                ||cart==VTR_RESULT_CONCRETE_MIXER||cart==VTR_RESULT_PLATFORM_TRAILER)
            {
                plistBuffer->camerasPic.nLicense_plate_type=VCA_BIGCAR_TYPE;
            }
            ...
            //车牌颜色
            plistBuffer->camerasPic.nLicense_plate_color=struITSPlateResult.struPlateInfo.byColor;
            int tcol;
            tcol=plistBuffer->camerasPic.nLicense_plate_color;
            if(tcol==VCA_BLUE_PLATE)
            {
                plistBuffer->camerasPic.nLicense_plate_color=COLOUR_BLUE;
            }
            else if(tcol==VCA_YELLOW_PLATE)
            {
                plistBuffer->camerasPic.nLicense_plate_color=COLOUR_YELLOW;
            }
            else if(tcol==VCA_WHITE_PLATE)
            {
                plistBuffer->camerasPic.nLicense_plate_color=COLOUR_WHITE;
            }
            else if(tcol==VCA_BLACK_PLATE)
            {
                plistBuffer->camerasPic.nLicense_plate_color=COLOUR_BLACK;
            }
            else if(tcol==VCA_GREEN_PLATE)
            {
                plistBuffer->camerasPic.nLicense_plate_color=COLOUR_GREEN;
            }
            else if(tcol==VCA_BKAIR_PLATE)
            {
                plistBuffer->camerasPic.nLicense_plate_color=COLOUR_BLACK;//民航黑色车牌
            }
            else if(tcol==VCA_RED_PLATE)
            {
                plistBuffer->camerasPic.nLicense_plate_color=COLOUR_RED;
            }
            else if(tcol==VCA_ORANGE_PLATE)
            {
                plistBuffer->camerasPic.nLicense_plate_color=COLOUR_ORANGE;
            }
            else if(tcol==VCA_OTHER)
            {
                plistBuffer->camerasPic.nLicense_plate_color=COLOUR_OTHER;
            }

            printf("plistBuffer->camerasPic.nLicense_plate_color=%d\n",plistBuffer->camerasPic.nLicense_plate_color);
            //车牌评分
              memcpy(&plistBuffer->camerasPic.nFaceScore ,struITSPlateResult.struPlateInfo.byBelieve,MAX_LICENSE_LEN);
              printf("plistBuffer->camerasPic.nFaceScore=%d\n",plistBuffer->camerasPic.nFaceScore);

            //程序推送照片时间
             time_t t;
             plistBuffer->camerasPic.tProgramTime = time(&t);

            //图片长度
            plistBuffer->camerasPic.nBackgroundPicLen=struITSPlateResult.struPicInfo[nBkTag].dwDataLen;
        ...
        //拷贝背景图片内存
        memcpy(plistBuffer->camerasPic.pBackBuffer, struITSPlateResult.struPicInfo[nBkTag].pBuffer,
               struITSPlateResult.struPicInfo[nBkTag].dwDataLen);
            plistBuffer->camerasPic.nFacePicLen = struITSPlateResult.struPicInfo[nLicenseTag].dwDataLen;
            memcpy(plistBuffer->camerasPic.pFaceBuffer, struITSPlateResult.struPicInfo[nLicenseTag].pBuffer,
            ...
            plistBuffer->camerasPic.sAlarmRect.fX= struITSPlateResult.struPicInfo[nBkTag].struPlateRect.fX;
            plistBuffer->camerasPic.sAlarmRect.fY=struITSPlateResult.struPicInfo[nBkTag].struPlateRect.fY;
            plistBuffer->camerasPic.sAlarmRect.fWidth=struITSPlateResult.struPicInfo[nBkTag].struPlateRect.fWidth;
            plistBuffer->camerasPic.sAlarmRect.fHeight=struITSPlateResult.struPicInfo[nBkTag].struPlateRect.fHeight;
...
bool HCCamera::OnSetupAlarmChan() {
    ...
    struAlarmParam.byLevel = 1;//²¼·ÀÓÅÏÈ¼¶£º0- Ò»µÈ¼¶£¨¸ß£©£¬1- ¶þµÈ¼¶£¨ÖÐ£©£¬2- ÈýµÈ¼¶£¨µÍ£©
    struAlarmParam.byRetVQDAlarmType = 1; //Prefer VQD Alarm type of NET_DVR_VQD_ALARM
    ...
    struAlarmParam.byAlarmInfoType = 1;  //ÖÇÄÜ½»Í¨±¨¾¯ÐÅÏ¢ÉÏ´«ÀàÐÍ£º0- ÀÏ±¨¾¯ÐÅÏ¢£¨NET_DVR_PLATE_RESULT£©£¬1- ÐÂ±¨¾¯ÐÅÏ¢(NET_ITS_PLATE_RESULT)
    struAlarmParam.bySupport = 4;
