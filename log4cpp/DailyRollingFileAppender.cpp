...
namespace log4cpp {
	...
	unsigned int myDailyRollingFileAppender::maxDaysToKeepDefault = 30;

	myDailyRollingFileAppender::myDailyRollingFileAppender(const std::string& name,
	...
	void myDailyRollingFileAppender::setMaxDaysToKeep(unsigned int maxDaysToKeep) {
		_maxDaysToKeep = maxDaysToKeep;
	}

	unsigned int myDailyRollingFileAppender::getMaxDaysToKeep() const {
		return _maxDaysToKeep;
	}

	void myDailyRollingFileAppender::rollOver()
	{
...
#define PATHDELIMITER "\\"
	...
	void myDailyRollingFileAppender::_append(const log4cpp::LoggingEvent &event)
	{
		struct tm now;
		time_t t = time(NULL);
   ...
   std::unique_ptr<Appender> create_daily_roll_file_appender(const FactoryParams& params)
   {
      ...
      return std::unique_ptr<Appender>(new myDailyRollingFileAppender(name, filename, max_days_keep, append, mode));
   }
}
